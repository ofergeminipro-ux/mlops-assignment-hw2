...
def verify_node(state: AgentState) -> dict:
    ...
    # Render the execution result safely
    exec_result = state.execution.render() if state.execution else "No result"
    ...
    # Parse the LLM's JSON defensively
    ...
    if match:
        try:
            ...
            issue = data.get("issue", "No explanation provided")
        except json.JSONDecodeError:
            is_ok = False
            issue = "Failed to parse verifier output"
    else:
        is_ok = False
        issue = "Verifier did not return JSON"
        
    # Hard-fail if the SQL engine threw an actual error
    if state.execution and state.execution.error:
        is_ok = False
        issue = f"SQL Execution Error: {state.execution.error}"

    return {"verify_ok": is_ok, "verify_issue": issue}
...
def revise_node(state: AgentState) -> dict:
    ...
    new_sql = _extract_sql(response.content)
    
    return {
        "sql": new_sql,
        "iteration": state.iteration + 1,
        "history": state.history + [{"node": "revise", "sql": new_sql}],
    }
